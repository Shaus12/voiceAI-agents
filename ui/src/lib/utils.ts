...
export function setStackRefreshCookie(refreshToken: string) {
  ...
  const isVoiceFXDomain = window.location.hostname.endsWith('.voicefx.com');
  const cookieDomainPart = isVoiceFXDomain ? '; domain=.voicefx.com' : '';
...
export async function impersonateAsSuperadmin(params: {
  ...
  // (served from the target sub-domain, e.g. app.voicefx.com). The route will set the
  ...
  // superadmin.voicefx.com we want to switch to app.voicefx.com. For any other domain
