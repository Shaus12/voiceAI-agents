...
async def calculate_workflow_run_cost(ctx, workflow_run_id: int):
    ...
    try:
        ...
        # Convert USD to VoiceFX AI Tokens (1 cent = 1 token)
        voicefx_tokens = round(float(cost_breakdown["total"]) * 100, 2)
        ...
        if org:
            ...
            try:
                ...
                await db_client.update_usage_after_run(
                    org_id, voicefx_tokens, duration_seconds, charge_usd
                )
                if charge_usd is not None:
                    logger.info(
                        f"Updated organization usage with ${charge_usd:.2f} USD ({voicefx_tokens} VoiceFX AI Tokens) and {duration_seconds}s duration for org {org_id}"
                    )
                else:
                    logger.info(
                        f"Updated organization usage with {voicefx_tokens} VoiceFX AI Tokens and {duration_seconds}s duration for org {org_id}"
                    )
            except Exception as e:
                logger.error(
        ...
        logger.info(
            f"Calculated cost for workflow run: ${cost_breakdown['total']:.6f} USD ({voicefx_tokens} VoiceFX AI Tokens)"
        )
