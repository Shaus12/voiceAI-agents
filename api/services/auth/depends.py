...
from api.constants import DEPLOYMENT_MODE, VOICEFX_MPS_SECRET_KEY, MPS_API_URL
    ...
    async with httpx.AsyncClient() as client:
        ...
        if DEPLOYMENT_MODE == "oss":
            ...
            response = await client.post(
                ...
                json={
                    "name": f"Default voiceFX Model Service Key",
            ...
            if not VOICEFX_MPS_SECRET_KEY:
                logger.warning(
                    "Warning: VOICEFX_MPS_SECRET_KEY not set for authenticated mode"
                )
                raise ValidationError("Missing VOICEFX_MPS_SECRET_KEY in non oss mode")
            ...
            response = await client.post(
                ...
                headers={"X-Secret-Key": VOICEFX_MPS_SECRET_KEY},
                timeout=10.0,
            )
        ...
        if response.status_code == 200:
            ...
            if service_key:
                ...
                configuration = {
                    "llm": {
                        "provider": ServiceProviders.VOICEFX.value,
                        "api_key": service_key,
                        "model": VoiceFXLLMModel.DEFAULT.value,  # Default model
                    },
                    "tts": {
                        "provider": ServiceProviders.VOICEFX.value,
                        "api_key": service_key,
                        "model": VoiceFXTTSModel.DEFAULT.value,  # Default model
                        "voice": VoiceFXVoice.DEFAULT.value,  # Default voice
                    },
                    "stt": {
                        "provider": ServiceProviders.VOICEFX.value,
                        "api_key": service_key,
                        "model": VoiceFXSTTModel.DEFAULT.value,  # Default model
                    },
                }
