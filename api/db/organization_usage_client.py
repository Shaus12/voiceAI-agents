...
class OrganizationUsageClient(BaseDBClient):
    ...
    async def _get_or_create_current_cycle_impl(
        ...
        stmt = insert(OrganizationUsageCycleModel).values(
            ...
            quota_voicefx_tokens=org.quota_voicefx_tokens,
        )
    ...
    async def check_and_reserve_quota(
        ...
        async with self.async_session() as session:
            ...
            result = await session.execute(
                ...
                .where(
                    and_(
                        ...
                        OrganizationUsageCycleModel.used_voicefx_tokens
                        + estimated_tokens
                        <= OrganizationUsageCycleModel.quota_voicefx_tokens,
                    )
                )
            ...
            if cycle_locked:
                ...
                cycle_locked.used_voicefx_tokens += estimated_tokens
                await session.commit()
                return True
    ...
    async def update_usage_after_run(
        ...
        async with self.async_session() as session:
            ...
            cycle_locked.used_voicefx_tokens += actual_tokens
    ...
    async def get_current_usage(self, organization_id: int) -> dict:
        ...
        async with self.async_session() as session:
            ...
            result = {
                ...
                "used_voicefx_tokens": cycle.used_voicefx_tokens,
                "quota_voicefx_tokens": cycle.quota_voicefx_tokens,
                "percentage_used": (
                    round(
                        (cycle.used_voicefx_tokens / cycle.quota_voicefx_tokens) * 100, 2
                    )
                    if cycle.quota_voicefx_tokens > 0
                    else 0
                ),
        ...
        async with self.async_session() as session:
            ...
            for run in runs:
                if run.cost_info:
                    ...
                    voicefx_tokens = run.cost_info.get("dograh_token_usage", 0)
                    # If not present, calculate from total_cost_usd (old format)
                    if voicefx_tokens == 0 and "total_cost_usd" in run.cost_info:
                        voicefx_tokens = round(
                ...
                run_data = {
                    ...
                    "dograh_token_usage": voicefx_tokens,
    ...
    async def get_daily_usage_breakdown(
        ...
        async with self.async_session() as session:
            ...
            total_minutes = 0
            total_cost_usd = 0
            total_voicefx_tokens = 0

            for row in daily_usage:
                seconds = row.total_seconds or 0
                minutes = seconds / 60
                cost_usd = seconds * price_per_second_usd
                voicefx_tokens = cost_usd * 100  # 1 cent = 1 token

                total_minutes += minutes
                total_cost_usd += cost_usd
                total_voicefx_tokens += voicefx_tokens
                ...
                breakdown.append(
                    {
                        ...
                        "voicefx_tokens": round(voicefx_tokens, 0),
            ...
            return {
                ...
                "total_voicefx_tokens": round(total_voicefx_tokens, 0),
                "currency": "USD",
            }
    ...
    async def update_organization_quota(
        ...
        async with self.async_session() as session:
            ...
            org.quota_voicefx_tokens = quota_voicefx_tokens
